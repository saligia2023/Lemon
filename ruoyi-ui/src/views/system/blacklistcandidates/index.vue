    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:blacklistcandidates:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:blacklistcandidates:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:blacklistcandidates:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:blacklistcandidates:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="blacklistcandidatesList" @selection-change="handleSelectionChange">
...
<script>
import { listBlacklistcandidates, getBlacklistcandidates, delBlacklistcandidates, addBlacklistcandidates, updateBlacklistcandidates } from "@/api/system/blacklistcandidates";

export default {
  name: "Blacklistcandidates",
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listBlacklistcandidates(this.queryParams).then(response => {
        this.blacklistcandidatesList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getBlacklistcandidates(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateBlacklistcandidates(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addBlacklistcandidates(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除黑名单编号为"' + ids + '"的数据项？').then(function() {
        return delBlacklistcandidates(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/blacklistcandidates/export', {
        ...this.queryParams
      }, `blacklistcandidates_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
